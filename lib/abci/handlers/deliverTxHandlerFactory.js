    ...
    switch (stateTransition.getType()) {
      ...
      case stateTransitionTypes.DOCUMENTS: {
        ...
        if (options.rateLimiterActive) {
          const ResponseDeliverTxWithRateLimiterCounter = new ResponseDeliverTx();
          ResponseDeliverTxWithRateLimiterCounter.code = 0;
          const limiter = new RateLimiter(options);
          const rateLimiterKey = limiter.getRateLimitedKey(blockchainState.getLastBlockHeight());
          const rateLimiterEvent = new KVPair(
            { key: rateLimiterKey, value: stateTransition.documents[0].userId },
          );
          ResponseDeliverTxWithRateLimiterCounter.tags = [rateLimiterEvent];
          response = ResponseDeliverTxWithRateLimiterCounter;
        }
