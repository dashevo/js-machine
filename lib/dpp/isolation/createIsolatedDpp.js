...
async function createIsolatedDpp(dataProvider, memoryLimit, timeout) {
  ...
  const createDataProviderCode = await compileFile('./createDataProviderWrapper', 'createDataProvider');
  const createDppCode = await compileFile('./createDpp', 'createDpp');
  const createTimeoutShim = await compileFile('./createTimeoutShim', 'createTimeoutShim');
  const isolateSnapshot = await Isolate.createSnapshot([
    { code: createDataProviderCode },
    { code: createDppCode },
    { code: createTimeoutShim },
  ]);
