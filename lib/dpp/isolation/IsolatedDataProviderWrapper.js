...
class IsolatedDataProviderWrapper {
  ...
  async fetchIdentity(id) {
    ...
    global.fetchIdentityRequest = id;

    // wait until id will be replaced by identity JSON
    // from main thread
    while (global.fetchIdentityRequest === id) {
      await wait(10);
    }

    // if we've found an identity return class instance
    if (global.fetchIdentityRequest) {
      return new Identity(global.fetchIdentityRequest);
    }

    // return null otherwise
    return global.fetchIdentityRequest;
