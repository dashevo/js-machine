...
function bootstrapIsolateFactory(dpp, snapshot, isolateOptions) {
  ...
  async function bootstrapIsolate() {
    ...
    await context.eval(`
      ...
      const { DashPlatformProtocol, ExternalDataProvider, createTimeoutShim } = DPP;
      ...
      const dataProvider = new ExternalDataProvider(dataProviderExternalReference)
      global.dataProviderWrapper = dataProvider;

      global.dpp = new DashPlatformProtocol({ dataProvider });
    `);
