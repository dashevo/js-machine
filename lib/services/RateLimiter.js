class RateLimiter {
  ...
  async isQuotaExceeded(userId, blockHeight) {
    ...
    const count = await this.fetchTransitionCountByEvent(limitedKey, userId, { prove: true });
  ...
  async isBannedUser(userId, blockHeight) {
    ...
    const count = await this.fetchTransitionCountByEvent(bannedKey, userId, { prove: true });
    return count > 0;
  }
