...
const compileFileWithBrowserify = require('./dpp/isolation/compileFileWithBrowserify');
const IsolatedDpp = require('./dpp/isolation/IsolatedDpp');
 ...
 * @param {string} options.ISOLATION_MEMORY_LIMIT
 * @param {string} options.ISOLATION_TIMEOUT_MILLIS
 *
...
async function createDIContainer(options) {
  ...
  container.register({
    ...
    isolationMemoryLimit: asValue(parseInt(options.ISOLATION_MEMORY_LIMIT, 10)),
    isolationTimeout: asValue(parseInt(options.ISOLATION_TIMEOUT_MILLIS, 10)),
  });
  ...
  const isolateDataProviderCode = await compileFileWithBrowserify(
    './dpp/isolation/internal/createDataProviderWrapper', 'createDataProvider',
  );
  const isolateDppCode = await compileFileWithBrowserify(
    './dpp/isolation/internal/createDpp', 'createDpp',
  );
  const isolateTimeoutShimCode = await compileFileWithBrowserify(
    './dpp/isolation/internal/createTimeoutShim', 'createTimeoutShim',
  );
